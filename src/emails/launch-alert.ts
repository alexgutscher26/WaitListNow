...
interface LaunchAlertEmailParams {
  name?: string;
  waitlistName?: string;
  message?: string;
}

export function getLaunchAlertEmail({ name, waitlistName, message }: LaunchAlertEmailParams) {
  const displayName = name ? `Hi ${name}! 👋` : 'Hi there! 👋';
  const wlName = waitlistName || 'our waitlist';
  const year = format(new Date(), 'yyyy');
  const launchMessage = message || `We're excited to announce that ${wlName} is now live! 🎉`;
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .header { background: linear-gradient(135deg, #10b981 0%, #3b82f6 100%); padding: 32px 24px; text-align: center; color: #fff; }
    ...
    <body>
      <div class=\"container\">
        <div class=\"email-wrapper\">
          <div class=\"header\">
            <h1>${wlName} is Live!</h1>
          </div>
          <div class=\"content\">
            <h2>${displayName}</h2>
            <p>${launchMessage}</p>
            <p>Thank you for being part of our journey. We can't wait for you to experience what we've built!</p>
...
Thank you for being part of our journey. We can't wait for you to experience what we've built!
