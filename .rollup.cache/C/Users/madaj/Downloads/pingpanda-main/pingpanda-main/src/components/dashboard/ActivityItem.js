...
var ActivityItem = function (_a) {
  ...
  return _jsxs('div', {
    ...
    children: [
      _jsx('div', { className: 'mt-1', children: _jsx(Icon, { className: 'h-6 w-6' }) }),
      _jsxs('div', {
        className: 'flex-1',
        children: [
          message,
          _jsx('div', {
            ...
            children: formatDistanceToNow(new Date(activity.time), { addSuffix: true }),
          }),
        ],
      }),
      _jsxs(DropdownMenu, {
        children: [
          _jsx(DropdownMenuTrigger, {
            ...
            children: _jsx('button', {
              ...
              children: _jsx(MoreHorizontal, { className: 'h-5 w-5 text-gray-500' }),
            }),
          }),
...
function getActivitySummary(activity) {
  switch (activity.type) {
    case 'new_subscriber':
      return ''
        .concat(activity.name, ' (')
        .concat(activity.email, ') joined ')
        .concat(activity.waitlist);
    case 'waitlist_created':
      return 'Waitlist "'
        .concat(activity.name, '" created with ')
        .concat(activity.subscribers, ' subscribers.');
    case 'referral':
      return ''
        .concat(activity.referrer, ' referred ')
        .concat(activity.referred, '. Reward: ')
        .concat(activity.reward);
    case 'conversion':
      return ''
        .concat(activity.name, ' converted from ')
        .concat(activity.waitlist, '. Revenue: $')
        .concat(activity.revenue);
    case 'milestone':
