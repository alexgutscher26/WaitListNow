...
var Page = function () {
    ...
    var createCheckoutSession = useMutation({
        mutationFn: function () { return __awaiter(void 0, void 0, void 0, function () {
            var res;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, client.payment.createCheckoutSession.$post({})];
                    case 1:
                        res = _a.sent();
    ...
    var handleGetAccess = function (planName) {
        ...
        if (user) {
            ...
            router.push("/sign-in?intent=upgrade&plan=".concat(encodeURIComponent(planName)));
        }
    };
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <MaxWidthWrapper className="py-12 md:py-20 relative z-10">
        ...
        <div className="mt-20 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {pricingPlans.map(function (plan, index) {
            var IconComponent = plan.icon;
            return (<div key={plan.name} className={"relative rounded-3xl bg-white/70 backdrop-blur-xl border border-white/20 shadow-xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 group ".concat(plan.featured
                    ? 'ring-2 ring-blue-500/50 scale-105 lg:scale-105 shadow-blue-500/25'
                    : 'hover:shadow-slate-500/25')} style={{
                    animationDelay: "".concat(index * 100, "ms"),
                    animation: 'fadeInUp 0.6s ease-out forwards',
                }}>
                {plan.badge && (<div className="absolute -top-5 left-1/2 transform -translate-x-1/2 z-10">
                    <div className={"bg-gradient-to-r ".concat(plan.color, " text-white text-sm font-bold px-6 py-2 rounded-full shadow-lg relative")}>
                ...
                <div className="p-8">
                  <div className={"inline-flex p-4 rounded-2xl bg-gradient-to-r ".concat(plan.color, " mb-6 shadow-lg group-hover:scale-110 transition-transform duration-300")}>
                  ...
                  <div className="mt-6 flex items-baseline gap-2">
                    ...
                    {plan.price !== 'Custom' && (<span className="text-slate-500 text-lg">/month</span>)}
                    {plan.originalPrice && (<span className="text-base text-slate-400 line-through ml-3 bg-red-50 px-2 py-1 rounded">
                        {plan.originalPrice}
                      </span>)}
                  </div>
                  ...
                  <Button onClick={function () { return handleGetAccess(plan.name); }} className={"w-full mt-8 h-14 font-semibold text-lg rounded-xl transition-all duration-300 ".concat(plan.featured
                    ? "bg-gradient-to-r ".concat(plan.color, " hover:opacity-90 text-white border-0 shadow-lg hover:shadow-xl hover:scale-105")
                    : 'hover:scale-105 shadow-md hover:shadow-lg')} variant={plan.buttonVariant} size="lg">
                    {plan.buttonText}
                ...
                <div className="border-t border-slate-200/50 bg-gradient-to-br from-slate-50/50 to-white/50 backdrop-blur-sm p-8 pt-6 rounded-b-3xl">
                  ...
                  <ul className="space-y-4">
                    ...
                    return (<li key={idx} className="flex items-start gap-3 text-sm text-slate-600">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gradient-to-r from-slate-50 to-blue-50">
                  <tr className="border-b border-slate-200">
                    ...
                    {pricingPlans.map(function (plan) { return (<th key={plan.name} className="py-6 px-4 font-semibold text-center min-w-[150px] text-gray-900 text-base">
                        <div className={"inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r ".concat(plan.color, " text-white text-sm")}>
                ...
                <tbody>
                  {features.map(function (feature, idx) { return (<tr key={feature.key} className={"border-b border-slate-100 hover:bg-slate-50/50 transition-colors ".concat(idx % 2 === 0 ? 'bg-white/30' : 'bg-slate-50/30')}>
                      <td className="py-5 px-6 font-medium text-gray-900">{feature.name}</td>
                      {pricingPlans.map(function (plan) { return (<td key={"".concat(plan.name, "-").concat(feature.key)} className="py-5 px-4 text-center">
                          {typeof plan.features[feature.key] ===
                    'boolean' ? (plan.features[feature.key] ? (<CheckCircle className="h-6 w-6 text-emerald-500 mx-auto"/>) : (<XCircle className="h-6 w-6 text-slate-300 mx-auto"/>)) : (<span className="font-semibold text-gray-900 bg-slate-100 px-3 py-1 rounded-full">
          ...
          <div className="bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600 rounded-3xl p-12 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                {testimonials.map(function (testimonial, index) { return (<div key={index} className="bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                    <div className="flex gap-1 mb-6">
                      {__spreadArray([], Array(5), true).map(function (_, i) { return (<Star key={i} className="h-6 w-6 fill-yellow-400 text-yellow-400"/>); })}
                    </div>
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <p className="text-slate-600 leading-relaxed text-lg">
                    ...
                    <a href="mailto:sales@waitlistnow.com" className="text-blue-600 hover:text-blue-700 font-semibold hover:underline transition-colors">
