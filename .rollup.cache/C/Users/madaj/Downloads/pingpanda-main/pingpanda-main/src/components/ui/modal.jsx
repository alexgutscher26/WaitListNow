...
export var Modal = function (_a) {
    ...
    if (isMobile && !desktopOnly) {
        return (<Drawer.Root open={setShowModal ? showModal : true} onOpenChange={function (open) {
                if (!open) {
        ...
        <Drawer.Portal>
          <Drawer.Content className={cn('fixed !max-w-none bottom-0 left-0 right-0 z-50 mt-24 rounded-t-[10px] border-t border-gray-200 bg-white', className)}>
    ...
    return (<Dialog open={setShowModal ? showModal : true} onOpenChange={function (open) {
            if (!open) {
                closeModal({ dragged: true });
            }
        }}>
