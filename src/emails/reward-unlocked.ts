...
interface RewardUnlockedEmailParams {
  name?: string;
  waitlistName?: string;
  rewardName: string;
  message?: string;
...
export function getRewardUnlockedEmail({
  ...
  const html = `
    ...
    <head>
      ...
      <title>Reward Unlocked - ${wlName}</title>
      <style>
        ...
        .header { background: linear-gradient(135deg, #f59e42 0%, #fbbf24 100%); padding: 32px 24px; text-align: center; color: #fff; }
      ...
      <div class=\"container\">
        <div class=\"email-wrapper\">
          ...
          <div class=\"content\">
            <h2>${displayName}</h2>
            <p>${rewardMessage}</p>
            <p>Thank you for sharing and helping us grow. Stay tuned for more rewards and updates!</p>
...
Congratulations! You've unlocked a new reward: ${rewardName} on ${wlName}.

Thank you for sharing and helping us grow. Stay tuned for more rewards and updates!
