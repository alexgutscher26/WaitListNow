...
export class WaitlistPreferencesError extends Error {
  constructor(message: string, public code: string = 'WAITLIST_PREFERENCES_ERROR') {
    super(message);
...
export async function getUserWaitlistPreferences(userId: string): Promise<WaitlistPreferences> {
  ...
  try {
    ...
    if (!prefsJson) {
      console.log(`[getUserWaitlistPreferences] No preferences found for user ${userId}, returning defaults`);
    ...
    try {
      ...
      console.error(`[getUserWaitlistPreferences] Error parsing preferences for user ${userId}:`, parseError);
    ...
    console.log(`[getUserWaitlistPreferences] Successfully retrieved preferences for user ${userId}`);
    return mergedPrefs;
  } catch (error) {
...
export async function updateUserWaitlistPreferences(
  ...
  try {
    console.log(`[updateUserWaitlistPreferences] Updating preferences for user: ${userId}`, { updates });
    ...
    const newPrefs = {
      ...
      maxSubscribers: Math.min(10000, Math.max(100, updates.maxSubscribers ?? currentPrefs.maxSubscribers)),
    };
    ...
    console.log(`[updateUserWaitlistPreferences] Successfully updated preferences for user ${userId}`);
    return newPrefs;
  } catch (error) {
