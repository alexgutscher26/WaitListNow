...
function deleteWaitlist(id) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch("/api/waitlists/".concat(id), {
                        method: 'DELETE',
                    })];
                case 1:
                    res = _a.sent();
...
export default function WaitlistsPage() {
    ...
    var _a = useQuery({
        ...
        queryFn: function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fetch('/api/waitlists', {
                            method: 'GET',
                    ...
                    case 2:
                        ...
                        throw new Error(errorData.message || "Failed to fetch waitlists (".concat(res.status, " ").concat(res.statusText, ")"));
                    case 3: return [4 /*yield*/, res.json()];
                    case 4:
                        data = _a.sent();
        ...
        retry: 1,
        retryDelay: 1500,
    }), _b = _a.data, waitlists = _b === void 0 ? [] : _b, isLoading = _a.isLoading, isError = _a.isError, error = _a.error, refetch = _a.refetch;
    // Delete mutation
    ...
    var activeWaitlists = safeWaitlists.filter(function (wl) { return wl.status === 'ACTIVE'; }).length;
    ...
    var lastUpdated = (function () {
        ...
        try {
            var sorted = __spreadArray([], safeWaitlists, true).filter(function (wl) { return wl.updatedAt; })
                .sort(function (a, b) {
    ...
    if (isError) {
        return (<DashboardPage title="Waitlists" description="Manage your waitlists" actions={<Button asChild>
        ...
        <div className="flex flex-col items-center justify-center space-y-4 py-12">
          ...
          <p className="text-sm text-muted-foreground">
            {(error === null || error === void 0 ? void 0 : error.message) || 'An unknown error occurred'}
          </p>
          <Button variant="outline" onClick={function () { return refetch(); }} className="mt-4">
    ...
    if (!isLoading && (!waitlists || waitlists.length === 0)) {
        return (<DashboardPage title="Waitlists" cta={<Button asChild>
            <Link href="/dashboard/waitlists/new">
              <Plus className="mr-2 h-4 w-4"/>
              New Waitlist
            </Link>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              {isLoading ? (<Skeleton className="h-8 w-16"/>) : (<div className="text-2xl font-bold">{(waitlists === null || waitlists === void 0 ? void 0 : waitlists.length) || 0}</div>)}
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (<Skeleton className="h-8 w-16"/>) : (<div className="text-2xl font-bold">{totalSubscribers.toLocaleString()}</div>)}
              <div className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (<Skeleton className="h-8 w-16"/>) : (<div className="text-2xl font-bold">{activeWaitlists}</div>)}
              <div className="text-xs text-muted-foreground">
                {isLoading ? (<Skeleton className="mt-2 h-4 w-32"/>) : (waitlists === null || waitlists === void 0 ? void 0 : waitlists.length) ? ("".concat(Math.round((activeWaitlists / ((waitlists === null || waitlists === void 0 ? void 0 : waitlists.length) || 1)) * 100), "% of total")) : ('No waitlists yet')}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (<Skeleton className="h-8 w-32"/>) : (<div className="text-2xl font-bold">{lastUpdated}</div>)}
              <div className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (<div className="space-y-4">
                {__spreadArray([], Array(3), true).map(function (_, i) { return (<WaitlistSkeleton key={i}/>); })}
              </div>) : hasWaitlists ? (<div className="space-y-4">
                {waitlists.map(function (waitlist) { return (<div key={waitlist.id} className="flex items-center justify-between rounded-lg border p-4 hover:bg-accent/50 transition-colors cursor-pointer" onClick={function () { return router.push("/dashboard/waitlists/".concat(waitlist.id)); }}>
                    <div className="flex items-center space-x-4">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={function (e) { return e.stopPropagation(); }}>
                    ...
                    router.push("/dashboard/waitlists/".concat(waitlist.id));
                        ...
                        <DropdownMenuItem onClick={function (e) {
                    e.stopPropagation();
                    var waitlistUrl = "".concat(window.location.origin, "/waitlist/").concat(waitlist.slug);
                    ...
                    backgroundColor: waitlist.status === 'ACTIVE'
                        ? '#10B981'
                        : waitlist.status === 'PAUSED'
                          ...
                          <DropdownMenuPortal>
                            <DropdownMenuSubContent>
                              {['DRAFT', 'ACTIVE', 'PAUSED', 'ARCHIVED'].map(function (status) { return (<DropdownMenuItem key={status} className={status === waitlist.status ? 'bg-accent' : ''} onClick={function (e) {
                        e.stopPropagation();
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem className="text-destructive" onClick={function (e) {
                    e.stopPropagation();
              ...
              </div>) : (<div className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
                <div className="rounded-full bg-muted p-4">
                  <Users className="h-8 w-8 text-muted-foreground"/>
                </div>
                <h3 className="text-xl font-semibold">No waitlists yet</h3>
