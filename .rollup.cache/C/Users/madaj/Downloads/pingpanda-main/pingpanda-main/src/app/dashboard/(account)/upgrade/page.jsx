    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 2:
                ...
                return [2 /*return*/, (<DashboardPage title="Upgrade Plan">
      <div className="space-y-16">
        ...
        <div className="grid lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {pricingPlans.map(function (plan) {
                            ...
                            return (<div key={plan.name} className={"relative rounded-2xl border bg-card text-card-foreground shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ".concat(plan.featured ? 'ring-2 ring-primary scale-105 lg:scale-110' : '')}>
                {plan.badge && (<div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
                    <span className={"bg-gradient-to-r ".concat(plan.color, " text-white text-sm font-semibold px-4 py-2 rounded-full shadow-lg")}>
                      {plan.badge}
                    </span>
                  </div>)}

                <div className="p-8">
                  <div className={"inline-flex p-3 rounded-xl bg-gradient-to-r ".concat(plan.color, " mb-6")}>
                  ...
                  <div className="mt-4 flex items-baseline gap-2">
                    ...
                    {plan.originalPrice && (<span className="text-sm text-muted-foreground line-through ml-2">
                        {plan.originalPrice}
                      </span>)}
                  </div>
                  ...
                  <Button className={"w-full mt-8 h-12 font-semibold ".concat(plan.featured
                                    ? "bg-gradient-to-r ".concat(plan.color, " hover:opacity-90 text-white border-0")
                                    : '')} variant={plan.buttonVariant} size="lg">
                    {plan.buttonText}
                  </Button>
                </div>
                ...
                <div className="border-t bg-muted/30 p-8 pt-6 rounded-b-2xl">
                  <ul className="space-y-4">
                    {features.slice(0, 6).map(function (feature) { return (<li key={feature.key} className="flex items-start gap-3 text-sm">
                        {typeof plan.features[feature.key] ===
                                        'boolean' ? (plan.features[feature.key] ? (<CheckCircle className="h-5 w-5 text-green-500 mt-0.5 flex-shrink-0"/>) : (<XCircle className="h-5 w-5 text-muted-foreground/40 mt-0.5 flex-shrink-0"/>)) : (<div className="flex items-center gap-3">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 p-8 rounded-2xl">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {testimonials.map(function (testimonial, index) { return (<div key={index} className="bg-background p-6 rounded-xl shadow-sm">
                <div className="flex gap-1 mb-4">
                  {__spreadArray([], Array(5), true).map(function (_, i) { return (<Star key={i} className="h-4 w-4 fill-yellow-400 text-yellow-400"/>); })}
                </div>
        ...
        <div className="bg-muted/30 p-8 rounded-2xl">
          ...
          <div className="max-w-3xl mx-auto">
            <div className="grid gap-4">
              {addOns.map(function (addon, index) { return (<div key={index} className="flex items-center justify-between p-6 bg-background rounded-xl shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h4 className="font-semibold">{addon.title}</h4>
                      {addon.type === 'one-time' && (<span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-medium">
                          One-time
                        </span>)}
                    </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2">
                  ...
                  {pricingPlans.map(function (plan) { return (<th key={plan.name} className="py-4 font-semibold text-center min-w-[120px]">
                      <div className="flex flex-col items-center gap-2">
                        <div className={"inline-flex p-2 rounded-lg bg-gradient-to-r ".concat(plan.color)}>
              ...
              <tbody>
                {features.map(function (feature, index) { return (<tr key={feature.key} className={"border-b hover:bg-muted/50 transition-colors ".concat(index % 5 === 0 && index > 0 ? 'border-t-2 border-muted' : '')}>
                    <td className="py-4 pr-4 font-medium">{feature.name}</td>
                    {pricingPlans.map(function (plan) { return (<td key={"".concat(plan.name, "-").concat(feature.key)} className="py-4 text-center">
                        {typeof plan.features[feature.key] ===
                                    'boolean' ? (plan.features[feature.key] ? (<CheckCircle className="h-5 w-5 text-green-500 mx-auto"/>) : (<XCircle className="h-5 w-5 text-muted-foreground/40 mx-auto"/>)) : (<span className="font-semibold text-primary">
                            {plan.features[feature.key]}
                          </span>)}
                      </td>); })}
                  </tr>); })}
