...
interface ProgressUpdateEmailParams {
  name?: string;
  waitlistName?: string;
  message: string;
}

export function getProgressUpdateEmail({ name, waitlistName, message }: ProgressUpdateEmailParams) {
  ...
  const html = `
    ...
    <head>
      ...
      <title>Update from ${wlName}</title>
      <style>
